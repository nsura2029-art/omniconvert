...
- The CAD shell action area below the file list is structured as three rows: (1) right-aligned `Convert all to` + select + `Clear all`, (2) right-aligned `Download all` (only when `allConverted`) sitting directly above the primary action, (3) full-width `Convert` (btn-primary). The `Download all` button must remain positioned immediately above `Convert` and must trigger the inline `buildZip` STORE bundle, not per-file downloads.
